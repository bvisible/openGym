...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onPairPrev, onPairNext, onSetRowRef, onProgressionSettings }) {
  ...
  const cell = (s, i, col, cls) => (
    <div className={'stp ' + cls}>
      {/* //// Neoffice — aria-label through t(): upstream hard-codes it in English,
          so a screen reader announced "Decrease" on a French journal. */}
  ...
  return <>
    ...
    {/* //// Neoffice — same at the simple level: no offer to CREATE a superset.
        An existing one keeps running, and its own controls stay. */}
    {!compact && showsSupersetControl(useStore.getState().S, false) && (onPairPrev || onPairNext) && <div className="row" style={{ gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>

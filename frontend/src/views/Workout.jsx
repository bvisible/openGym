...
function StartChooser() {
  ...
  return <div className="narrow">
    ...
    <div className="hdr">
      ...
      <div style={{ flex: 1, marginLeft: 10 }}><h1>{t('Start workout')}</h1><div className="sub">{t(DAYN[new Date().getDay()])} — {todayRoutines.length ? t('today is {0}', todayName) : t('rest day, but no one’s stopping you')}</div></div>
    </div>
...
function ExerciseBlock({ entryIdx, compact, dense, onToggle, onToggleSide, onRest, onField, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onPairPrev, onPairNext, onSetRowRef, onProgressionSettings, onSwap, onMoveUp, onMoveDown, canMoveUp, canMoveDown, onRemoveExercise, busy }) {
  const S = useStore(s => s.S)
  ...
  const openMore = () => menuSheet({
    ...
    items: [
      ...
      //// Neoffice — gated on the detail level (lib/level-visibility.js): a beginner's menu
      //// does not offer warm-up ramps or supersets; one already in use keeps its controls.
      showsInSessionWarmup(S, entry.sets.some(isWarmupRow)) && { icon: 'flame', label: t('Add warm-up set'), onClick: onAddWarmup },
      onPairPrev && showsSupersetControl(S, false) && { icon: 'link', label: t('Make superset with previous'), onClick: onPairPrev },
      onPairNext && showsSupersetControl(S, false) && { icon: 'link', label: t('Make superset with next'), onClick: onPairNext },
  ...
  const openSetMenu = (s, i) => {
    ...
    menuSheet({
      ...
      items: [
        //// Neoffice — same gate as the chips: intensifiers are offered from the complete level, kept once a row carries some.
        !warm && mode === 'reps' && !isRestPauseSet(s) && showsSetIntensifierChips(S, dropsOf(s).length > 0 || clustersOf(s).length > 0) && { icon: 'arrowDown', label: t('Drop set'), sub: t('+ Drop'), onClick: () => addDropRow(i) },
        !warm && mode === 'reps' && !isDropSet(s) && showsSetIntensifierChips(S, dropsOf(s).length > 0 || clustersOf(s).length > 0) && { icon: 'bolt', label: t('Rest-pause burst'), sub: t('+ Burst'), onClick: () => addBurstRow(i) },
  ...
  return <>
    ...
    <div className="row between" style={{ marginBottom: 6 }}>
      ...
      <div className="capitalize" style={{ fontSize: (compact || dense) ? 17 : 20, fontWeight: 600, letterSpacing: '-.02em', lineHeight: 1.2 }}>{exerciseNameFor(ex)}</div>
    ...
    {wc.pairButtons && !compact && !dense && showsSupersetControl(useStore.getState().S, false) && (onPairPrev || onPairNext) && <div className="row" style={{ gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
    ...
    <div className="card" style={{ marginTop: 10, marginBottom: 0 }}>
      ...
      {entry.sets.map((s, i) => {
        ...
        return <div key={i}>
          ...
          {!warm && mode === 'reps' && <>
            ...
            {/* //// Neoffice — gated on the detail level as well as the switch: a member
                //// on Simple was still offered "+ Drop" and "+ Burst" under every set. A row
                //// that already carries drops or clusters keeps its chips. */}
            {wc.setShortcuts && showsSetIntensifierChips(S, dropsOf(s).length > 0 || clustersOf(s).length > 0) && <div className="setextra">
      ...
      {wc.setShortcuts ? <div className="row" style={{ flexWrap: 'wrap' }}>
        {/* //// Neoffice — warm-up ramps are a taught concept: offered from Normal up, kept once the exercise has warm-up rows. */}
...
function ActiveWorkout() {
  ...
  //// screen (which offered no way to add, 02.09). The picker closes once the
  //// exercise is in the session: left open, a member who had just added one
  //// was still looking at the list and could not see what they had added.
  const addExerciseFlow = () => {
    const picker = exercisePicker((ex, quick) => {
      ...
      const commit = cfg => { picker.close(); update(s => {
  ...
  return <div className="narrow">
    ...
    {A.entries.length ? (listMode ? (
      ...
      {/* //// Neoffice — the empty freestyle screen offers the add itself. */}
      <div style={{ marginTop: 14 }}><Button variant="primary" icon="plus" onClick={addExerciseFlow}>{t('Add exercise')}</Button></div>
    </div>}

    <div style={{ height: 12 }} />
    {/* //// Neoffice — centred like every row below it; left-hugging made the screen look accidental. */}
    {!listMode && <div className="row" style={{ justifyContent: 'center' }}>

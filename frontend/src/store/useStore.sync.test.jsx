...
//// URL. Ours calls NAMED helpers (getState/getRev/putState/logout): the journal
//// is served from Frappe and talks to whitelisted methods, not to paths. Left
//// unmocked they come back undefined, pullState() throws into its own catch,
//// and a test reads as "kept the local state" — the very bug it exists to
//// catch, reported for the wrong reason. The helpers below delegate to the same
//// mocked api() with UPSTREAM's paths and verbs, so every assertion written
//// against '/api/data', 'PUT' and `baseRev` holds unchanged: the test double
//// speaks upstream's protocol, the real helpers speak Frappe's. `who` is what
//// the page boot says about the member (currentUser), upstream's /api/me.
...
describe('ordering', () => {
  ...
  it('boot keeps a change made during its pull and pushes the merge', async () => {
    ...
    //// Neoffice — who we are comes from the page boot, not from /api/me.
    who.user = { id: 'user-1', name: 'One' }

...
//// Neoffice — upstream mocks api() alone because its store calls api() with a
//// URL. Ours calls NAMED helpers (getState/putState/logout): the journal is
//// served from Frappe and talks to whitelisted methods, not to paths. Left
//// unmocked they come back undefined, pullState() throws into its own catch,
//// and the test reads as "kept the local state" — which is exactly the bug it
//// exists to catch, reported for the wrong reason.
//// They delegate to the same mocked api(), so api.mockResolvedValue() in each
//// test still drives them and the assertions on call counts stay true.
vi.mock('../lib/api.js', () => {
  const api = vi.fn()
  return {
    api,
    getState: () => api('state.get'),
    putState: (state) => api('state.put', { method: 'POST', body: JSON.stringify({ state }) }),
    logout: () => api('logout', { method: 'POST', body: '{}' }),
    currentUser: () => null,
    setRemoteAuth: () => {},
  }
})
...
describe('saved workout state sync and restore', () => {
  ...
  it('pushes local data instead of replacing it when the local state is newer', async () => {
    ...
    //// Neoffice — upstream asserts the path '/api/data'; ours goes through a
    //// whitelisted Frappe method, so the second call is identified by being a
    //// POST (the push) rather than by its URL. What the test is really about —
    //// that the LOCAL state is the one sent — is the assertion below, and it
    //// is unchanged.

...
//// Neoffice — Nora's model may run on the club's own hardware: the long budget, like a local endpoint.
export const timeoutFor = provider => (provider === 'compatible' || provider === 'nora' ? LOCAL_ENDPOINT_TIMEOUT_MS : TIMEOUT_MS)
...
const cfgOf = d => (d.mode === 'nora' || d.provider === 'nora') ? noraCfg() : ({ provider: d.provider, providerOptions: d.baseUrl ? { [d.provider]: { baseUrl: d.baseUrl } } : {} })
//// Neoffice — the transport per mode: the phone's native HTTP for a key of its own, the page's own fetch (session cookie) for Nora.
const fetchFor = d => (d.mode === 'nora' ? noraFetch : nativeFetch)
...
export async function localDisclosure() {
  ...
  return {
    ...
    //// Neoffice — with Nora it is the club's: the model is theirs and the calls are counted for them.
    payer: d.mode === 'nora' ? 'instance' : 'you', host: hostOf(base)
  }
}
...
async function start(S, kind, opts) {
  ...
  if (!(d.mode === 'byok' || d.mode === 'nora') || !adapter) throw Object.assign(new Error(t('The Coach isn’t set up on this phone.')), { status: 503, code: 'off' })
  //// Neoffice — with Nora the club's own limits apply, counted on the instance

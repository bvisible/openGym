...
describe('the gate has something real to read', () => {
  it('the store actually exposes the `config` field every Coach entry point reads', async () => {
    ...
    //// Neoffice — the field is filled from the boot blob (window.gym_boot.coach), not
    //// from GET /api/config: there is no Node server behind this journal.
    expect(src).toMatch(/gym_boot/)
    expect(src).toMatch(/boot\.coach/)

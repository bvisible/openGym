...
//// 'img/' resolves to nothing on a Frappe instance. The media/ folder is NOT in
//// the repository (see .gitignore) — the instance downloads it with
//// scripts/fetch-media.sh, the way upstream's `docker compose up` does for a
//// self-hoster. The environment variables still win: that is what lets a build
//// point at a CDN.
const IMG_BASE = ENV.VITE_IMG_BASE || '/assets/opengym/media/img/'
const GIF_BASE = ENV.VITE_GIF_BASE || '/assets/opengym/media/gif/'

//// Neoffice — a club can put ITS OWN media on an exercise. An absolute URL
//// ("/files/…", set from the desk) is served as is; a bare file name stays a
//// library one and gets the folder prefix. The test is on the SHAPE of the
//// value, not on a flag: the journal has no business knowing where a media
//// comes from, only where to fetch it.
...
//// muscles stay the library's. Applied on EXIDX (the runtime index), which is
